...
import { runEnrichment } from '../src/enrichment/engine';
...
describe('Artist Enrichment (Phase 3)', () => {
  it('should find un-enriched artists and populate metadata', async () => {
    ...
    db.prepare('INSERT INTO shows (id, venue_id, artist_name, event_date) VALUES (?, ?, ?, ?)').run('1', 'cactus-club', 'The Gufs', '2026-03-01');
  ...
  it('should handle enrichment failures gracefully', async () => {
    const db = initDb(':memory:');
    db.prepare('INSERT INTO venues (id, name) VALUES (?, ?)').run('cactus-club', 'Cactus Club');
    db.prepare('INSERT INTO shows (id, venue_id, artist_name, event_date) VALUES (?, ?, ?, ?)').run('1', 'cactus-club', 'Unknown Band', '2026-03-01');
    ...
    const metaCount = db.prepare('SELECT COUNT(*) as count FROM artists_metadata').get().count;
