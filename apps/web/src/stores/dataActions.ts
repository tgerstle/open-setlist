...
export async function loadMonth(monthKey: string) {
	if (
		loadedMonthsStore.get().includes(monthKey) ||
		isLoadingPastStore.get() ||
		isLoadingFutureStore.get()
	)
		return false;

	const isPast =
		loadedMonthsStore.get().length > 0 && monthKey < loadedMonthsStore.get()[0];
...
export async function fetchOlderMonth() {
	...
	if (currentFloor > earliestLoadedFloor) {
		...
		announceToScreenReader(
			`Showing events starting from ${new Date(earliestLoadedFloor).toLocaleDateString("en-US", { month: "long", year: "numeric" })}`,
		);
	...
	if (index > 0) {
		...
		if (success) {
			...
			announceToScreenReader(
				`Loaded older events from ${new Date(`${prevMonth}-01`).toLocaleDateString("en-US", { month: "long", year: "numeric" })}`,
			);
			return true;
		}
	}
...
export async function loadSpecificMonths(months: string[]) {
	...
	try {
		...
		if (newEvents.length > 0) {
			...
			loadedMonthsStore.set(
				[...loadedMonthsStore.get(), ...successfullyLoaded].sort(),
			);
		}
	} catch (err) {
