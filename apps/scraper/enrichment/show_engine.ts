...
export const runShowEnrichmentPass = async (db: Database.Database) => {
	...
	for (const show of pendingShows) {
		try {
			...
			if (!extractor) {
				console.log(
					`[Skipping] No detail extractor for ${show.venue_id}, marking as completed.`,
				);
...
const isMainModule =
	...
	process.argv[1].endsWith("show_engine.ts");
