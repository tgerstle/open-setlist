...
(async () => {
	...
	const uniqueViewports = new Set(
		results.map((r) => JSON.stringify(r.viewport)),
	).size;

	console.log(
		`🕵️ Unique User-Agents: ${uniqueUAs}/${results.length} ${uniqueUAs > 1 ? "✅" : "⚠️"}`,
	);
	console.log(
		`🌐 Unique IPs: ${uniqueIPs}/${results.length} ${uniqueIPs > 1 || !process.env.PROXY_SERVER ? "✅" : "❌"}`,
	);
	console.log(
		`📐 Unique Viewports: ${uniqueViewports}/${results.length} ${uniqueViewports > 1 ? "✅" : "⚠️"}`,
	);
})();
