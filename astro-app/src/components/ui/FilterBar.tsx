...
export function FilterBar({ initialVenues }: FilterBarProps) {
  ...
  const activeBgClass = activeVenueCtx ? `bg-${activeThemeColor}-100` : "bg-white";

  // Dynamic body background tied to the active venue color
  useEffect(() => {
    // Extract the -50 variant from the pill string (e.g. "bg-blue-50 text-...")
    const bodyBgClass = activeVenueCtx && activeThemeColor ? `bg-${activeThemeColor}-50` : "bg-[#f6f5f4]";
    
    // Remove existing bg- classes
    const classesToRemove = Array.from(document.body.classList).filter(c => c.startsWith("bg-"));
            ...
            >
              ...
              >
                <div className="flex items-center justify-between px-4 py-3 border-b border-[rgba(0,0,0,0.1)] flex-shrink-0">
                  <h3 id="modal-title" className="font-bold text-lg">Filters</h3>
                  <button
                    onClick={() => setIsOpen(false)}
