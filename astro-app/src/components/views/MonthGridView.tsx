...
import { fetchOlderMonth, fetchNewerMonth } from "../../stores/dataActions";
...
export function MonthGridView({
  ...
  useEffect(() => {
    // Avoids hydration mismatch from server's new Date vs client Date
  ...
  const sortedMonths = useMemo(() => {
    if (loadedMonths.length === 0 && initialShows.length > 0) {
      const distinct = Array.from(new Set(initialShows.map(s => (s.date || s.event_date || "").substring(0, 7)).filter(Boolean)));
      return distinct.sort();
    }
    return [...loadedMonths].sort();
  }, [loadedMonths, initialShows]);
  ...
  const calendarMonths = useMemo(() => {
    ...
    return sortedMonths.map((monthStr) => {
      ...
      const monthName = MO_NAMES[monthIndex];
  ...
  const canLoadOlder = loadedMonths.length > 0 && availableMonths.indexOf(loadedMonths[0]) > 0;
  const canLoadNewer = loadedMonths.length > 0 && availableMonths.indexOf(loadedMonths[loadedMonths.length - 1]) < availableMonths.length - 1;
    ...
    <div className="flex flex-col h-full gap-10 relative pb-10">
        ...
        <div className="flex justify-center sticky top-0 z-30 pt-2 pb-4">
          <button
            ...
            className={`px-6 py-2 bg-white/90 backdrop-blur-sm border shadow-sm text-[#59647a] font-bold text-[10px] uppercase tracking-widest rounded-full transition-all active:scale-95 ${
              isLoadingPast ? "opacity-50 cursor-not-allowed" : "hover:bg-slate-50 border-slate-200"
            }`}
          >
        ...
        >
          ...
          <div className="bg-slate-50 border-b border-slate-100 py-3 px-4 flex justify-between items-end">
            <h2 className="text-xl font-bold tracking-tight text-[#1e293b]">
              {calMonth.monthName} <span className="text-slate-400 font-medium ml-1">{calMonth.year}</span>
            </h2>
          </div>
          ...
          <div className="grid grid-cols-7 border-b border-[rgba(0,0,0,0.05)] bg-white text-xs font-semibold text-[#59647a]/70 tracking-[0.125px]">
            {weekDays.map((day) => (
              <div key={day} className="py-2 text-center uppercase border-r border-[rgba(0,0,0,0.02)] last:border-r-0">
                {day}
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-7 auto-rows-fr bg-[#f6f5f4] gap-[1px]">
            {calMonth.days.map((cell, i) => {
              if (!cell) {
                // Empty padding cell
                return <div key={`pad-${i}`} className="bg-[#fafafa] min-h-[120px] lg:min-h-[140px]" />;
              }
                ...
                >
                  ...
                  <div className="mb-2">
                    <span
                      className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-sm font-semibold tracking-[-0.125px] ${
                        isToday ? "bg-[#0052a3] text-white shadow-md" : "text-[#31302e]"
                      }`}
                    >
                      {cell.day}
                    </span>
                  ...
                  <div className="space-y-0.5 max-h-[160px] overflow-y-auto no-scrollbar pr-0.5">
                    {cell.shows.slice(0, 4).map((show) => (
                      <ShowPill key={show.id} show={show} onClick={() => onShowClick(show)} />
                    ))}
      ...
      <DayEventsModal
        ...
        onNextDay={() => {
          ...
          const currentIndex = allDaysList.findIndex((d) => d?.dateStr === selectedDayEvents.dateStr);
