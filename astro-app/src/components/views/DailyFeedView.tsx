...
export function DailyFeedView({
  ...
  if (shows.length > 0) {
    if (
      ...
      const oldIndex = shows.findIndex((s) => s.id === prevFirstShowIdRef.current);
      if (oldIndex > 0) {
      ...
      >
        <GroupedVirtuoso
          ...
          rangeChanged={(range) => {
            ...
            const topShow = groupedShows[Math.max(0, range.startIndex - firstItemIndex)];
          ...
          itemContent={(index) => {
            ...
            if (!show) {
               console.warn(`No show found for virtuoso absolute index ${index} (arrayIndex: ${arrayIndex})`);
               return null;
            }
            return (
