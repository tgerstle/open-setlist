...
export const getPendingShows = (db: Database.Database, limit: number = 100) => {
  const stmt = db.prepare(`SELECT * FROM shows WHERE enrichment_status = 'pending' AND detail_url IS NOT NULL LIMIT ?;`);
  return stmt.all(limit) as any[];
};

export const updateShowEnrichment = (db: Database.Database, id: string, data: any) => {
  const fields = [];
  const values = [];
